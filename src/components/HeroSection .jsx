...
const HeroSection = () => {
  ...
  const email = "Dhiraj.Prajapati@gmail.com";
 ...
 useGSAP(() => {
  ...
  .from(".subtitle", {
    ...
    duration: 0.8,
    ease: "power2.out",
  }, "-=0.2") // slight overlap for smoother flow
  .from(".button", {
    y: 100,
    opacity: 0,
    duration: 0.4,
  ...
  .from(".title span", {
    ...
    stagger: 0.1,
    ease: "power2.out",
  }, "-=0.8"); // starts just before button finishes
  ...
  return (
    <div ref={containerRef} className="relative z-10 container mx-auto px-4 md:px-6 pt-14">
      <div className="max-w-6xl mx-auto text-center">
        ...
        <div  className="tagline inline-flex items-center gap-2 mt-20 px-3 py-1 rounded-full bg-white/5 border border-white/10 mb-8 md:mb-12">
          <span className="bg-[#9810F2] px-1.5 text-white  rounded-full -ml-1.5">new</span>
          <div className="shiny-text animate-pulse text-white/80">Coming soon...</div>
        </div>
        ...
        <div  className="button animate-fadeInUp mt-4 flex flex-col items-center justify-center gap-6 md:mt-8 md:flex-row md:gap-10">
          ...
          <div className="relative">
            <button className="flex items-center gap-2 py-3 text-base font-light text-white/75 outline-none transition-all duration-300 cursor-pointer hover:text-white/90">
           { copy ?   <Copy className="w-5 h-5" onClick={handleCopy}/>: <Check className="w-5 h-5" /> }
             {copy ?email:message}
            </button>
          </div>
        </div>
